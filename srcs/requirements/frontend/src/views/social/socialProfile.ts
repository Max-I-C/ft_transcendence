...
export function showProfilePopup(profile: { username: string; game_play: number; game_win: number; game_loss: number; score_total: number; level: number; is_online: number; avatar?: string }, pos?: { x: number; y: number }) {
    ...
    popup.innerHTML = `
        <div class="profile-popup-inner">
            ...
            <h3>
                <img id="avatar-img" src="${profile.avatar ? `http://localhost:3000${profile.avatar}` : '/views/images/default.png'}" alt="Avatar" width="100" height="100" />
                @${profile.username}
